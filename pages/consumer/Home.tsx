...
const projectToSingapore = (lat: number, lng: number) => {
  const x =
    ((lng - SINGAPORE_BOUNDS.minLng) /
      (SINGAPORE_BOUNDS.maxLng - SINGAPORE_BOUNDS.minLng)) *
    100;
  const y =
    (1 -
      (lat - SINGAPORE_BOUNDS.minLat) /
        (SINGAPORE_BOUNDS.maxLat - SINGAPORE_BOUNDS.minLat)) *
    100;
...
const SingaporeMapView: React.FC<{
  bags: SurpriseBag[];
  vendors: Vendor[];
}> = ({ bags, vendors }) => {
  const points: MapPoint[] = bags
    .map((bag) => {
      ...
      return {
        bag,
        vendor,
        position: projectToSingapore(vendor.lat, vendor.lng),
      };
    })
...
const ConsumerHome: React.FC = () => {
  ...
  if (isLoading || vendorsLoading)
    return <div className="text-center py-10">Finding delicious deals...</div>;
          ...
          <div className="pt-4 border-t border-gray-100 grid md:grid-cols-3 gap-6 animate-in slide-in-from-top-2">
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {["Meals", "Bread & Pastries", "Groceries", "Dessert"].map(
                  (c) => (
                    <button
                      key={c}
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {["Vegetarian", "Vegan", "Halal-Friendly", "Gluten-Free"].map(
                  (d) => (
                    <button
                      key={d}
          ...
          <AnimatePresence mode="popLayout">
            {filteredBags?.map((bag: SurpriseBag, index: number) => {
              const vendor = vendors?.find(
                (v: Vendor) => v.id === bag.vendorId
              );
              const savings = (
                ((bag.originalPrice - bag.price) / bag.originalPrice) *
                100
              ).toFixed(0);
              return (
                <motion.div
                  ...
                  <Link to={`/consumer/bags/${bag.id}`}>
                    ...
                    >
                      ...
                      >
                        <img
                          ...
                          onError={(e) => {
                            ...
                            if (!target.src.includes("1504674900247")) {
                              target.src = `https://images.unsplash.com/photo-1504674900247-0877df9cc836?w=800&h=600&fit=crop`;
                            }
                          }}
                          loading="eager"
                        />
                      ...
                      <div className="p-5 flex flex-col flex-grow">
                        ...
                        <h4 className="font-semibold text-gray-800 mb-3 text-base line-clamp-2">
                          {bag.title}
                        </h4>
