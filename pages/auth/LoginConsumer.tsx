...
const LoginConsumer: React.FC = () => {
  ...
  // If already logged in (e.g., returning from Google OAuth), redirect away
  useEffect(() => {
    console.log("LoginConsumer render", { isLoading, user });
    if (!isLoading && user) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      const user = await login(email, password, UserRole.CONSUMER);
